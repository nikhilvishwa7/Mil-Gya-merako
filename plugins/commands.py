...
async def start(client, message):
    ...
    if data.startswith("sendfiles"):
        ...
        k = await client.send_message(chat_id=message.from_user.id,text=f"<b>Get All Files in a Single Click!!!\n\n📂 ʟɪɴᴋ ➠ : {g}\n\n<i>Note: This message is deleted in 5 mins to avoid copyrights. Save the link to Somewhere else</i></b>", reply_markup=InlineKeyboardMarkup(
                [
                    [
                        InlineKeyboardButton('📁 join Update 📁', url=g)
                    ], [
                        InlineKeyboardButton('🤔 Hᴏᴡ Tᴏ Buy 🤔', url=await get_tutorial(chat_id))
                    ]
                ]
            )
        )
        ...
        for file in files:
            ...
            if not await check_verification(client, message.from_user.id) and VERIFY == True:
                btn = [[
                    InlineKeyboardButton("Buy", url=f'https://t.me/JNGohell"))
                ]]
                await message.reply_text(
    ...
    msg = await client.send_cached_media(
        ...
        reply_markup=InlineKeyboardMarkup(
             [
              InlineKeyboardButton('🫨ᴍᴏᴠɪᴇ ɢʀᴏᴜᴘ', url=f'https://t.me/i_movieee') 
           ]
        )
    )
    btn = [[
